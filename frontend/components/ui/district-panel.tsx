                        ...
                        <div className="p-8 rounded-[32px] bg-white/[0.02] border border-white/5">
                            ...
                            <div className="space-y-4">
                                {ngos.length > 0 ? (
                                        ...
                                        return (
                                            <div key={idx} className={`overflow-hidden rounded-xl border transition-all duration-300 ${isExpanded ? "bg-white/[0.07] border-white/20" : "bg-white/5 border-white/5"}`}>
                                                <div className="flex justify-between items-center p-4 cursor-pointer" onClick={() => setExpandedNgo(isExpanded ? null : ngo.org_name)}>
                                                    <div>
                                                        {/* --- Increased NGO Name to text-lg --- */}
                                                        <p className="text-lg font-bold text-white">{ngo.org_name}</p>
                                                        {/* --- Increased NGO Type to text-xs --- */}
                                                        <p className="text-xs text-white/40 uppercase tracking-widest mt-1">{ngo.org_type || "NGO"}</p>
                                                    </div>
                                                    <div className="flex items-center gap-4">
                                                        {/* --- Increased Action text to text-xs --- */}
                                                        <span className="text-xs font-bold text-[#138808] uppercase tracking-tighter">{isExpanded ? "Close" : "View Impact"}</span>
                                                            ...
                                                            <div className="pt-4 grid grid-cols-2 gap-4">
                                                                <div className="space-y-3">
                                                                    <div className="flex items-center gap-2">
                                                                        ...
                                                                        {/* --- Increased detail font to text-xs --- */}
                                                                        <p className="text-xs text-white/40 uppercase tracking-widest">Founded: <span className="text-white font-semibold">{ngo.yearFounded || "2018"}</span></p>
                                                                    </div>
                                                                    <div className="flex items-center gap-2">
                                                                        <Users className="w-4 h-4 text-[#FF9933]" />
                                                                        <p className="text-xs text-white/40 uppercase tracking-widest">Beneficiaries: <span className="text-white font-semibold">{ngo.beneficiaries || "500+"}</span></p>
                                                                    </div>
                                                                </div>
                                                                <div className="space-y-3">
                                                                    <div className="flex items-center gap-2">
                                                                        <Target className="w-4 h-4 text-[#138808]" />
                                                                        <p className="text-xs text-white/40 uppercase tracking-widest">Focus: <span className="text-white font-semibold">Education</span></p>
                                                                    </div>
                                                                    <div className="flex items-center gap-2 cursor-pointer group">
                                                                        <Globe className="w-4 h-4 text-[#138808]" />
                                                                        <p className="text-xs text-white/40 uppercase tracking-widest group-hover:text-white transition-colors">Website: <span className="text-white underline font-semibold">{ngo.website ? "Visit" : "N/A"}</span></p>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </motion.div>
                                    ...
                                    <p className="text-white/40 text-sm italic">No NGOs currently registered in this district.</p>
                                )}
                            </div>
                        </div>
                    </div>
